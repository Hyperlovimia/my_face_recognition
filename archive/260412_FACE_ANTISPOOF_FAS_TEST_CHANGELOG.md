...
将上述文件拷到板子可读写路径（示例统一写 **`/sharefs/`**，实际可按项目规范调整）。
...
1. **确认文件已到位**（路径替换为你的实际路径）：
   ...
   ```text
   msh /> ls /sharefs/fas_test.elf
   msh /> ls /sharefs/face_antispoof.kmodel
   msh /> ls /sharefs/test.png
   ```

2. **可执行权限**（若需要）：

   ```text
   msh /> chmod +x /sharefs/fas_test.elf
   ```

3. **执行测试**（**两个参数**：先 kmodel，后图片）：

   ```text
   msh /> /sharefs/fas_test.elf /sharefs/face_antispoof.kmodel /sharefs/test.png
   ```
...
本节面向 **需要在 PC 上从 ONNX 产出 kmodel** 的同事（集成/算法）。当前仓库通过 **`.gitignore`** 忽略 **`utils/face_antispoof.kmodel`**、**`models_src/prep_debug.npy`**、**`models_src/calib_npy/`**、**`models_src/nncase_dump/`** 等生成物，**克隆仓库后不会自带 kmodel**，须按下列步骤在本地生成，再将 **`face_antispoof.kmodel`** 发给板端同事或拷贝到板子（如 `/sharefs/face_antispoof.kmodel`）。
