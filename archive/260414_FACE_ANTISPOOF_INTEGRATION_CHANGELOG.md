...
- 功能与行为概览
- 详细变更项（文件、接口、策略）
- `face_ai` 启动参数与 RT-Smart 板端手动启动方式
...
| 阈值 | `REAL` 概率默认阈值为 **0.5**；代码仍保留读取 **`FACE_FAS_REAL_THRESH`** 的能力，但 RT-Smart `msh` 通常不能通过 `export` 设置环境变量，板端部署不要依赖该方式。 |
...
| 启动方式 | RT-Smart 板端不使用启动脚本。启用活体时手动给 `face_ai.elf` 追加第 9 参数 `/sharefs/face_antispoof.kmodel`；不启用活体时保持 8 参数。 |
...
- 删除 **`utils/run_face3.sh`**、**`utils/watchdog_face3.sh`** 与旧 **`utils/run.sh`**，避免误导板端使用 shell 脚本自动启动。
- RT-Smart `msh` 不能按常规 Linux shell 使用，不应依赖 `export`、变量展开、命令替换、shell 函数、循环、`trap` 或通用进程管理命令。
- 启用活体时，部署人员在串口中直接给 **`face_ai.elf`** 追加第 9 参数；不启用活体时直接使用 8 参数。
...
| **`utils/run_face3.sh` / `utils/watchdog_face3.sh` / `utils/run.sh`** | 依赖类 Linux shell 能力，不能作为 RT-Smart 板端启动入口，已移除。 |
...
活体 REAL 概率阈值默认为 **0.5**。源码仍保留读取 **`FACE_FAS_REAL_THRESH`** 的能力，但 RT-Smart `msh` 通常无法使用 `export` 设置进程环境变量，因此板端现场默认按 **0.5** 使用；如需改阈值，建议改源码默认值后重新编译，或后续扩展为命令行参数。

### 6.3 RT-Smart 三进程启动顺序
...
- `70`：识别相似度阈值
- `0`：调试模式关闭
- 最后一个可选参数为活体模型路径，存在且有效时启用活体；省略时不启用活体
...
日志路径由启动 **`face_event.elf`** 时的参数指定，例如 **`/tmp/attendance.log`**；将路径配置到 SD 卡即可持久化 **翻拍/攻击留痕**。
...
本次变更在保留 **`FaceAntiSpoof`** 与训练一致预处理的前提下，将 **活体结果写入 `face_video` 应答（OSD）** 与 **`face_event` 事件通道**：**活体未通过** 时单独上报 **`LIVENESS_FAIL`**，在考勤日志中体现为 **`[SPOOF]`**，与陌生人、已识用户事件区分，满足 **攻击留痕** 与后续 **网络推送** 扩展需求；并对 **注册入库** 施加 **活体门控**。同时 **移除独立 `fas_test` 工具链、仓库内校准样例图以及不适用于 RT-Smart 的启动脚本**，部署与维护路径收敛到 **`face_ai.elf` + 可选第 9 参数**；**`utils/face_antispoof.kmodel`** 可由仓库跟踪并手工同步至板端 **`/data`** 等部署目录。
